...
if (process.env.REACT_APP_ENABLE_MOCK) {
  ...
  mock.onGet('/ecosystems/123ds-1231qwsdfsd-12eqadfgs/objects').reply(200, objectsExampleData)
}
...
export function fetchObjects({ customer, ecosystem}) {
  console.log({customer, ecosystem})
  return rest.get(`/ecosystems/${ecosystem}/objects`).then(response => response.data)
}
